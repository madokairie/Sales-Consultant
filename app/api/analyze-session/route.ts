...
事前教育レベル: ${config.preEducationLevel === 'full' ? '教育済み（価格まで公開）' : config.preEducationLevel === 'partial' ? '説明済み（価格未公開）' : config.preEducationLevel === 'none' ? '事前説明なし' : '不明'}
${config.preEducationDetail ? `事前教育の詳細: ${config.preEducationDetail}` : ''}
===ここまで===
...
- 事前教育レベルに対して適切な説明量だったか確認してください（教育済みなのに長々と商品説明していないか、未教育なのに説明が足りていないか）
