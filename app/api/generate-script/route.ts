...
成約率80%以上を実現するトップセールスの知見をもとに、個別相談（1on1セールス）のスクリプトを設計してください。

【スクリプト設計の原則】
1. 「売る」のではなく「買いたくなる」状態を作る
...
【フェーズ構成】
1. opening（アイスブレイク）: 緊張をほぐし信頼関係の土台を作る
2. hearing（ヒアリング）: 現状・課題・願望を引き出す質問
3. deepening（深掘り）: 課題の本質に迫り、痛みを明確化する
4. vision（理想の未来）: 解決後の理想状態を一緒に描く
5. presentation（解決策提示）: 商品/サービスを課題解決の手段として自然に紹介
6. proof（実績・証明）: 同じ悩みを持った人の成功事例
7. offer（オファー）: 価格・条件の提示（アンカリング活用）
8. objection（反論処理）: 想定される断り文句への切り返し
9. closing（クロージング）: 自然な流れで申込へ
...
断り文句が提供されている場合、objectionフェーズでそれぞれに対する切り返しトークを必ず含めてください。`,
        },
      ],
    });
